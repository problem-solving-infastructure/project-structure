...
For example, for LeetCode, the division will be into `Easy`, `Medium`, and `Hard'.
...
In addition to explicitly dividing the files into different sections, name the files using the task number (if any). Programming languages like Dart contain a basic linter rule for naming files, where it is forbidden to use numbers in the file name. If you don't need the rule except in this case, disable it in the linter settings.

We need this rules for easy viewing your files. If you are serching solution from LeetCode, it is easy to navigate by problem number, not its name.
